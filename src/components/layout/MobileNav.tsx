...
export function MobileNav() {
  ...
  const { dueCount, dailyDue } = useNavBadges();
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-ink-900/80 pb-[env(safe-area-inset-bottom)] backdrop-blur-2xl lg:hidden">
      <div className="mx-auto flex max-w-lg items-stretch justify-around px-2">
        {MOBILE_NAV.map((item) => {
          ...
          const showReview = item.badge === "review" && dueCount > 0;
          const showDaily = item.badge === "daily" && dailyDue;
            ...
            >
              {isLab ? (
                ...
                <span className="relative">
                  ...
                  />
                  {showReview && (
                    <span className="absolute -right-2 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-sky px-1 text-[9px] font-bold text-white">
                      {dueCount}
                    </span>
                  )}
                  {showDaily && (
